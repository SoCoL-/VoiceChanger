...
/**
 * Created by Evgenij on 25.12.13.
 * Обработчик запросов и ответов сервака
 */
public class RequestService implements OnFinishTask
{
    //-----------------------------
    //Constants
    //-----------------------------
    ...
    public RequestService(Context c, IResponce resp)
    {
        this.mContext = c;
        this.mIResponce = resp;
    ...
    {
        HttpSocket task = new HttpSocket(r, this, mContext);
        task.execute(0);
    }

    private void parseError(String status, String TAG, String mess)
    {
        String message = mContext.getString(R.string.Error_server_service_not_support);
        if(status.equals("-1"))
        {
            message = mContext.getString(R.string.Error_server_service_not_support);
        }
        else if(status.equals("23"))
        {
            if(mess.contains("CaptchaCode"))
                message = mContext.getString(R.string.Error_server_23_captcha);
            else if(mess.contains("SmsCode"))
                message = mContext.getString(R.string.Error_server_23_sms);
        }
        else if(status.equals("40"))
        {
            ...
            {
                TAG = "Banned";
                message = mContext.getString(R.string.Error_server_40);
                Settings.isBanned = true;
            }
        }
        else if(status.equals("41"))
        {
            message = mContext.getString(R.string.Error_server_41);
        }
        else if(status.equals("42"))
        {
            message = mContext.getString(R.string.Error_server_42);
        }
        else if(status.equals("43"))
        {
            message = mContext.getString(R.string.Error_server_43);
        }
        else if(status.equals("46"))
        {
            message = mContext.getString(R.string.Error_server_46);
        }
        else if(status.equals("47"))
        {
            message = mContext.getString(R.string.Error_server_47);
        }
        ...
        {
            message = mContext.getString(R.string.Error_server_60);
        }
        else if(status.equals("63"))
        {
            message = mContext.getString(R.string.Error_server_63);
        }
        else
        {
            message = mContext.getString(R.string.Error_server_service_not_support);
            ...
            if(!r.isError())
            {
                ...
                if(status.equals(TYPE_OK))
                {
                    ...
                    if(event.equals(Settings.TAG_EVENT_CALL_A))
                        Settings.mCallMessage = mContext.getString(R.string.event_call_A);
                    else if(event.equals(Settings.TAG_EVENT_CALL_B))
                        Settings.mCallMessage = mContext.getString(R.string.event_call_B);
                    else if(event.equals(Settings.TAG_EVENT_FAIL_A))
                    {
                        Settings.mCallMessage = mContext.getString(R.string.event_fail_call_A);
                        Settings.isStopUpdate = true;
                    }
                    else if(event.equals(Settings.TAG_EVENT_FAIL_B))
                    {
                        Settings.mCallMessage = mContext.getString(R.string.event_fail_call_B);
                        Settings.isStopUpdate = true;
                    }
                    else if(event.equals(Settings.TAG_EVENT_PICKUP_A))
                        Settings.mCallMessage = mContext.getString(R.string.event_pickup_A);
                    else if(event.equals(Settings.TAG_EVENT_PICKUP_B))
                        Settings.mCallMessage = mContext.getString(R.string.event_pickup_B);
                    else if(event.equals(Settings.TAG_EVENT_DISCARD_A))
                    {
                        Settings.mCallMessage = mContext.getString(R.string.event_discard_A);
                        Settings.isStopUpdate = true;
                    }
                    else if(event.equals(Settings.TAG_EVENT_FINAL))
                    {
                        Settings.mCallMessage = mContext.getString(R.string.event_final);
                        Settings.isStopUpdate = true;
                    }
                    else if(event.equals(Settings.TAG_EVENT_WAIT))
                        Settings.mCallMessage = mContext.getString(R.string.event_wait);

                    String time = getString(obj, ANSW_LENGTH);
                    if(!time.equals("0"))
                        Settings.mCallMessage += " " + mContext.getString(R.string.hint_call_length) + " " + time + " " + mContext.getString(R.string.hint_call_sec);
    ...
    private void getCaptcha(JSONObject o)
    {
        ...
        {
            Settings.mType = getString(objCaptchaData, ANSW_TYPE);
            if(Settings.mType.equals("code"))
            {
                Settings.mTextCaptcha = getString(objCaptchaData, ANSW_DATA);
                Settings.mData = null;
            }
            else
            {
                String data = getString(objCaptchaData, ANSW_DATA);
                Settings.mData = Base64.decode(data.getBytes(), Base64.DEFAULT);
            }
            mSaver.saveCaptcha();
        }
    }
