...
public class AdapterVoice extends BaseAdapter
{
    ...
    private ArrayList<MediaPlayer> mPlayer;
    ...
    public AdapterVoice(Context c/*, MediaPlayer mp*/)
    {
        this.mContext = c;
        /*this.mPlayer = mp;*/
        mPlayer = new ArrayList<MediaPlayer>(10);
        for(int i = 0; i < 10; i++)
            mPlayer.add(new MediaPlayer(mContext));
    ...
    public View getView(final int position, View convertView, ViewGroup parent)
    {
        ...
        mPlay.setOnClickListener(new View.OnClickListener()
        {
            ...
            {
                if(!mPlayer.get(position).isPlay())
                {
                    //mPlayer.get(position).stopPlayer();
                    for(int i = 0; i < 10; i++)
                        mPlayer.get(i).stopPlayer();

                    mPlayer.get(position).PlayMusic(Settings.getVoiceByID(position));
        ...
        mPlayer.get(position).setCopleteInterface(ocl);
