...
public class MainActivity extends Activity implements IResponce, OnClearData
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        mUpdateStatus = new Runnable()
        {
            ...
            public void run()
            {
                ...
                jsonFields.put(AppService.REQ_CALL_ID, Settings.mCurrCallID);
        ...
        mBtnCall.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                if(isServiceBind && Settings.isConnectInternet)
                {
                    ...
                    jsonFields.put(AppService.REQ_MSISDN_A, Settings.mMSISDN);
                    //noinspection ConstantConditions
                    Debug.i("Номер из записной книжки = " + mMsisdnB);
                    jsonFields.put(AppService.REQ_MSISDN_B, mMsisdnB);
                    jsonFields.put(AppService.REQ_VOICE_ID, "" + (Settings.mVoiceID+1));
                    String locale = "EN";
        ...
        mBtnCallSMS.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                ...
                it.putExtra(Settings.TAG_SMS_BODY, Settings.TAG_SMS_CALL + " " + mMsisdnB + " голосом " + Settings.getNameByID(MainActivity.this, Settings.mVoiceID));
                startActivity(it);
            }
        });
    ...
    public void onAnswerOk(String TAG)
    {
        ...
        if(Settings.mJSON != null && Settings.mJSON.length() > 0 && Settings.mSignature != null && Settings.mSignature.length() > 0)
        {
            ...
            jsonFields1.put(AppService.REQ_RESPONCE_DATA, Settings.mJSON);
            jsonFields1.put(AppService.REQ_SIGNATURE, Settings.mSignature);
            String data = android.util.Base64.encodeToString(new JSONObject(jsonFields1).toString().getBytes(), android.util.Base64.NO_WRAP);
            HashMap<String, String> jsonFields = new HashMap<String, String>();
            jsonFields.put(AppService.REQ_PURCHASE_DATA, data);
    ...
    protected void onResume()
    {
        ...
        if(Settings.isHelpCall /*&& Settings.isHelpCallSMS*/ && Settings.mCurrMethod != 5 && isServiceBind)
        {
            Settings.mCurrMethod = 5;
            mService.mSaver.saveHelpState();
        }
